...
public class WebSocketConnection  extends WebSocketListener {
    ...
    public void connect() {
        synchronized(this.mutex) {
            if (null == this.webSocket) {
                logger.info("[Connection {}] Connecting to {}", this.connectionId, this.streamName);
                this.webSocket = client.newWebSocket(this.request, this);
            } else {
                logger.info("[Connection {}] is already connected to {}", this.connectionId, this.streamName);
            }
    ...
    public void close() {
        if (null != this.webSocket) {
            logger.info("[Connection {}] Closing connection to {}", this.connectionId, this.streamName);
    ...
    public void onOpen(WebSocket ws, Response response) {
        logger.info("[Connection {}] Connected to Server", this.connectionId);
        this.onOpenCallback.onReceive((String)null);
    }
    ...
    public void onFailure(WebSocket ws, Throwable t, Response response) {
        logger.error("[Connection {}] Failure", this.connectionId, t);
        this.onFailureCallback.onReceive((String)null);
    }
}
